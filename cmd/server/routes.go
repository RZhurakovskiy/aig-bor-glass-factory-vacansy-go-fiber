...
<head>
	...
	<style>
		...
		* { box-sizing: border-box; }
		body { margin: 0; font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", "Helvetica Neue", Arial, sans-serif; min-height: 100vh; display: grid; place-items: center; background: linear-gradient(135deg, #111, #303030); color: #fff; }
		form { width: min(420px, calc(100% - 32px)); padding: 32px; border-radius: 16px; background: rgba(255,255,255,0.08); backdrop-filter: blur(16px); box-shadow: 0 24px 60px rgba(0,0,0,0.28); }
		h1 { margin-top: 0; margin-bottom: 12px; font-size: 32px; }
		p { margin: 0; line-height: 1.5; color: rgba(255,255,255,0.76); }
		label { display: grid; gap: 8px; margin: 20px 0 18px; font-weight: 700; }
		input { padding: 14px 16px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.14); background: rgba(255,255,255,0.96); color: #161616; font-size: 16px; }
		input:focus { outline: none; border-color: rgba(236,0,0,0.55); box-shadow: 0 0 0 3px rgba(236,0,0,0.15); }
		...
		.login-status { min-height: 22px; margin-top: 8px; font-size: 14px; color: rgba(255,255,255,0.78); }
		.login-status.is-error { color: #ffd6d1; }
